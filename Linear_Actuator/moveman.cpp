...
#include "../../Pingbot_serial14/ball_tracking/SerialClass.h"
#include <sstream>
...
void moveman::move_manipulator(float send_x, float send_y, float send_z, float send_vx, float send_vy, float send_vz, float send_t) //원래 manipulator 움직이는 함수
{
   ...
   // Serial open
	...
	//cout << command1 << endl;	
	//makecom makecom;
	string p1 = to_string(send_x);
	string p2 = to_string(send_y);
	string p3 = to_string(send_z);
	string r = to_string(rad);
	string n = to_string(send_t);;
	char com[100] = "jointandtool";
